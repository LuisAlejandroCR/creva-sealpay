// signature.ts: proves a report came from Creva, not merely that its content did not change.
// The trusted key is always the verifier's own; reading it from the document being checked
// would let a forger verify perfectly against themselves.
// Vendored verbatim from creva_finance/backend/src/common/integrity/signature.ts — see ../PROVENANCE.md.
