// signing-key.ts: loads the signing keys from a file or an env value, never into a log.
// A PEM written into a dotenv line loses its line breaks, so a file path wins when both exist.
// Vendored verbatim from creva_finance/backend/src/common/integrity/signing-key.ts — see ../PROVENANCE.md.
