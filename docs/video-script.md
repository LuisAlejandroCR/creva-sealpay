...
| Precio | No hay precio — el ask es la pista de premio: Hedera ($6k) + World ($3.5k) |
...
**No grabar la liquidación real de Hedera como si ya existiera.** `docs/plan.md` (bloque
"Gateway x402/Hedera") dice, a la fecha de este guion, que el cliente del facilitador ya habla el
formato real de BlockyDevs pero **todavía no hay una transacción real liquidada** — falta llave
pagadora o JWT de Bazantic. Si eso sigue así el día de grabar, el guion de abajo usa lenguaje que
es cierto sin esa pieza ("el gateway pide pago, listo para liquidar en Hedera testnet") — no
"pagó", no "liquidó". Corregir esta nota en cuanto haya una tx real (ver `docs/memoria.md`).
...
| 1:45–2:05 | **Honestidad sobre lo que falta** | *"Lo que no hemos hecho todavía: liquidar un pago real contra Hedera testnet — el gateway ya habla el protocolo, falta la llave. Se los mostramos en vivo si el jurado quiere verlo."* (Ajustar si ya existe tx real para ese día — mostrar el hash.) |
| 2:05–2:30 | **El ask** | *"Aplicamos a Hedera AI & Agentic Payments y a World Selfie Check, dentro de la pista Continuity. Les pedimos avanzar como finalistas — lo que ya corre es la prueba de que el resto se cierra."* |
